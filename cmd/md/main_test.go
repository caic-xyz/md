...
const (
	fakeDiffRuntimeEnv     = "MD_TEST_FAKE_DIFF_RUNTIME"
	fakeDiffSSHEnv         = "MD_TEST_FAKE_DIFF_SSH"
	fakeDiffSSHLogEnv      = "MD_TEST_FAKE_DIFF_SSH_LOG"
	fakePruneRuntimeEnv    = "MD_TEST_FAKE_PRUNE_RUNTIME"
	fakePruneRuntimeLogEnv = "MD_TEST_FAKE_PRUNE_RUNTIME_LOG"
)

func TestMain(m *testing.M) {
	if os.Getenv(fakeDiffSSHEnv) == "1" && strings.TrimSuffix(filepath.Base(os.Args[0]), ".exe") == "ssh" {
		ctx, stop := signal.NotifyContext(context.Background(), os.Interrupt)
		exitCode := runFakeDiffSSH(ctx, os.Args[1:])
		stop()
		os.Exit(exitCode)
	}
	if os.Getenv(fakeDiffRuntimeEnv) == "1" {
		os.Exit(runFakeDiffRuntime(os.Args[1:]))
	}
...
func runFakeDiffSSH(ctx context.Context, args []string) int {
	hostIndex := 0
	...
	command := strings.Join(args[hostIndex+1:], " ")
	if logPath := os.Getenv(fakeDiffSSHLogEnv); logPath != "" {
		f, err := os.OpenFile(logPath, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o600) //nolint:gosec // private test path.
		if err != nil {
	...
	cmd := exec.CommandContext(ctx, "bash", "-c", command) //nolint:gosec // fake SSH executes commands generated by the test target.
...
func runFakeDiffRuntime(args []string) int {
	if len(args) == 2 && args[0] == "inspect" && args[1] == "md-test" {
	...
	_, _ = fmt.Fprintf(os.Stderr, "unexpected fake diff runtime command: %s\n", strings.Join(args, " "))
	return 1
}
...
func TestDiffCommandError(t *testing.T) {
	...
	t.Run("differences", func(t *testing.T) {
		t.Parallel()
		err := diffCommandError(fmt.Errorf("repository: %w", md.ErrDiffFound), true)
		exitErr, ok := errors.AsType[*exitCodeError](err)
		if !ok || exitErr.code != 1 {
			t.Fatalf("diffCommandError() = %v, want silent exit code 1", err)
		}
	})
	t.Run("failure_without_exit_status", func(t *testing.T) {
		t.Parallel()
		want := errors.New("remote Git failed")
		if got := diffCommandError(want, false); !errors.Is(got, want) {
			t.Fatalf("diffCommandError() = %v, want original error", got)
		}
	})
	t.Run("failure_with_exit_status", func(t *testing.T) {
		t.Parallel()
		want := errors.New("remote Git failed")
		got := diffCommandError(want, true)
		exitErr, ok := errors.AsType[*exitCodeError](got)
		if !ok || exitErr.code != 2 || !errors.Is(got, want) {
			t.Fatalf("diffCommandError() = %v, want diagnostic exit code 2 wrapping %v", got, want)
		}
	})
}
...
func TestCmdDiff(t *testing.T) { //nolint:paralleltest // environment selects re-entered fake executables.
	t.Run("quiet_setup_failure_exits_two", func(t *testing.T) { //nolint:paralleltest // initLogging mutates the global logger.
		...
		err := a.cmdDiff(t.Context(), []string{"-repo=" + t.TempDir(), "--quiet"})
		exitErr, ok := errors.AsType[*exitCodeError](err)
		if !ok || exitErr.code != 2 || !strings.Contains(err.Error(), "not in a git repository") {
			t.Fatalf("cmdDiff() = %v, want repository diagnostic with exit code 2", err)
		}
	})
	t.Run("all_quiet_checks_every_repository", func(t *testing.T) { //nolint:paralleltest // setup changes the process environment.
		...
		if stdout.Len() != 0 || stderr.Len() != 0 {
			t.Fatalf("quiet output = stdout %q, stderr %q; want empty", stdout.String(), stderr.String())
		}
		log, readErr := os.ReadFile(logPath) //nolint:gosec // private test path.
		if readErr != nil {
			t.Fatal(readErr)
		}
		if got := strings.Count(string(log), "GIT_OPTIONAL_LOCKS=0"); got != 2 {
			t.Fatalf("final diff command count = %d, want 2:\n%s", got, log)
		}
	})
}

func setupCmdDiffContainer(t *testing.T, repoCount int) (ct *md.Container, logPath string) {
	exe, err := os.Executable()
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := copyTestExecutable(exe, filepath.Join(binDir, sshName)); err != nil {
		t.Fatal(err)
	}
	logPath = filepath.Join(t.TempDir(), "ssh.log")
	t.Setenv(fakeDiffSSHEnv, "1")
	t.Setenv(fakeDiffSSHLogEnv, logPath)
	...
	ct = &md.Container{
		Client: &md.Client{Home: home, Logger: logger, Runtime: rt},
		Logger: logger,
		Name:   "md-test",
		State:  "running",
	}
	for i := range repoCount {
		...
		runMainTestGit(t, "", "init", "-q", "--bare", "--initial-branch=main", originDir)
		runMainTestGit(t, hostDir, "init", "-q", "--initial-branch=main")
		runMainTestGit(t, hostDir, "config", "user.name", "Test")
		runMainTestGit(t, hostDir, "config", "user.email", "test@test")
		tracked := filepath.Join(hostDir, fmt.Sprintf("repo-%d.txt", i))
		if err := os.WriteFile(tracked, []byte("base\n"), 0o600); err != nil {
			t.Fatal(err)
		}
		runMainTestGit(t, hostDir, "add", ".")
		runMainTestGit(t, hostDir, "commit", "-q", "-m", "base")
		runMainTestGit(t, hostDir, "remote", "add", "origin", originDir)
		runMainTestGit(t, hostDir, "push", "-q", "-u", "origin", "main")
		runMainTestGit(t, "", "clone", "-q", originDir, containerDir)
		runMainTestGit(t, containerDir, "config", "user.name", "Test")
		runMainTestGit(t, containerDir, "config", "user.email", "test@test")
		if err := os.WriteFile(filepath.Join(containerDir, filepath.Base(tracked)), []byte("different\n"), 0o600); err != nil {
			t.Fatal(err)
		}
		runMainTestGit(t, containerDir, "commit", "-q", "-am", "container change")
		runMainTestGit(t, hostDir, "remote", "add", "md-test", containerDir)
		ct.Repos = append(ct.Repos, md.Repo{
			GitRoot:       hostDir,
