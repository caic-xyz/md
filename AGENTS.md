...
Smoke tests verify end-to-end image build, container launch, cache injection, nested podman inside a sudo-enabled container, and md startup on a foreign base image. They are gated behind the `smoke` build tag:
...
- **Localtime**: md bind-mounts the host `/etc/localtime` into the container. Base images must keep `/etc/localtime` a regular file, not a symlink: mounting over a symlink follows it and clobbers `/usr/share/zoneinfo/Etc/UTC`, breaking `TZ=UTC` inside the container. The final `RUN` in `rsc/root/Dockerfile` enforces this after every `apt` step.
...
## Container Startup Capabilities

`rsc/specialized/root/start.sh` treats every subsystem beyond SSH as a capability instead of assuming the
bundled image's packages exist. A capability is either:
...
- `build.go`: Build support prepares embedded resources and validates target platforms.
...
- `containers/base.go`: Runtime base implements shared Docker and Podman CLI operations.
...
- `example_test.go`: Examples demonstrate using md as a library.
...
- `rsc/specialized/root/start.sh`: Container entrypoint: md runs this as root to prepare the container, then idles.
