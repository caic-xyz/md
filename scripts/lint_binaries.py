...
# Files that are legitimately executable without a shebang (e.g. XFCE .desktop launchers).
ALLOWED_EXECUTABLE_WITHOUT_SHEBANG = {".desktop"}


def is_binary(file_path):
    """Return True if the file contains null bytes in its first 1024 bytes."""
...
def is_executable(file_path):
    """Return True if the file has the owner executable bit set."""
    try:
...
def main():
    """Check all git-tracked files for unexpected binaries and executables."""
    try:
    ...
    for f in files:
        ...
        if is_executable(f) and not has_shebang(f) and ext not in ALLOWED_EXECUTABLE_WITHOUT_SHEBANG:
    ...
    if unexpected_executables:
        print("Error: unexpected executable files (missing shebang) in repository:")
